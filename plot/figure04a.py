"""Figure 4a: variant of figure04.py with panel a replaced.

Run from the repo root: `python plot/figure04a.py`.

Identical to figure04.py (same b/c panels, same shared legend, same
filtering) except panel a: instead of the cost-function-vs-walk-run-
transition-speed panel, this shows metabolicCostUmberger (a THIRD metabolic
model neither OCP was optimized for) vs. speed across the sweep, effort vs.
metabolic (Bhargava) cost, bounded to METABOLIC_COST_SWEEP_BOUNDS
(0.83-4 m/s) -- see draw_metabolic_cost_sweep_panel's docstring for why 4
m/s specifically cuts off before the more striking part of this story (the
two curves cross around 3 m/s and reverse by ~5 m/s).

This session's earlier panel-a design (triceps surae/tibialis anterior
activation vs. an experimental EMG reference from the Lorenz & van den
Bogert (2024) Zenodo perturbation dataset, 10.5281/zenodo.10557913) is kept
in the module (draw_muscle_activation_panel and everything it depends on)
but no longer wired to the 'a' slot -- see that function's own docstring,
and MUSCLE_INDEX's comment, for the BioMAC-Sim-Toolbox verification work and
the EMG extraction pipeline, still valid if that panel's wanted again
(e.g. in a figure04b.py).
...
its own, since it's read together with 'a'):
    a) cost function vs. Umberger-scored metabolic cost across the speed
       sweep (0.83-4 m/s), effort exp.-3 vs. Bhargava metabolic cost, each
       annotated at its own minimum-cost speed (the model's own
       cost-of-transport optimum).
...
def _draw_condition_curves(ax, curves, colors, reference=None, linestyle='-', align=True):
    """Mean +/- SD curve per condition in `curves` (color from `colors`),
    plus an optional (mean, std) reference curve drawn behind everything
    else. Shared by panel b (effort/metabolic), panel d (footwear variants),
    and the muscle-activation panel so all three read the same way.

    When `align` is True (the default), each condition's trials are
    circularly shifted (in frames) to their MAE-minimising alignment against
    the reference mean before averaging -- the same `_best_shift` figure01.py's
    own reference overlays use, and that data_exploration_main.ipynb's
    plot_variability used before that. This matters for panel b/d because the
    sim's 0%-gait-cycle convention (right heelstrike, see
    gait_loading._make_heelstrike_first_node) is not guaranteed to match the
    reference source's own phase convention -- most visibly for the
    Fukuchi-derived running reference (panel d), whose 0%-event isn't
    documented anywhere in this repo's pipeline, but this is applied
    unconditionally rather than only when a mismatch is suspected.

    `align=False` (used by the muscle-activation panel's EMG reference) skips
    this: that reference's 0%-event (right heel-strike, see
    load_reference_emg) was deliberately built to already match the sim's own
    convention, so there's no real ambiguity to correct for. Checked this
    session: applying the MAE-shift there anyway produces a large, spurious
    realignment (up to 39% of the cycle) for a near-zero simulated curve
    (metabolic-cost tibialis anterior) where the MAE surface is almost flat
    -- the 'best' shift is essentially noise, not a genuine phase correction,
    and visually implies timing agreement that isn't really there.
    """
    ref_mean = None
    ...
    for key, trials in curves.items():
        ...
        if ref_mean is not None and align:
...
# ---------------------------------------------------------------------------
# Panel a: cost function -- triceps surae/tibialis anterior activation at the
# shared walking speed. Placed after draw_cost_grf_panel (not at the top of
# the file, despite being panel 'a') because it reuses GRF_SHAPE_SPEED and
# _draw_condition_curves defined above -- same non-alphabetical-order
# pattern the free-speed panel below already uses (code section "Panel b"
# is actually the third visual column).
# ---------------------------------------------------------------------------

# 0-based index into the 92-muscle activation-state order, one per component
# muscle (both gastrocnemius heads + soleus, combined below into a single
# strength-weighted "triceps surae" curve per user request, plus tibialis
# anterior on its own). VERIFIED against the actual toolbox that produced
# these results, BioMAC-Sim-Toolbox (NOT PredSim, which is only the baseline
# comparator elsewhere in this repo -- an earlier pass this session wrongly
# reverse-engineered PredSim's OCP_formulation.m/state layout, which doesn't
# apply here at all): on asm-biomac-ws01, loaded a raw simulations/*.mat's
# `result.problem.model` (a Gait3d_smoothsphere instance) and called its own
# `extractState('a')`/`.states.name` -- the model's own ground-truth
# name-to-index map, not a guess. That confirmed:
#   * model.extractState('a') returns exactly 92 indices, MATLAB 1-based
#     159:250 -> Python 0-based 158:249 -- i.e. gait_loading.py's existing
#     `muscle_indices = range(158, 250)` was already correct.
#   * names_a in order (0-based within the 92-muscle block): 31=med_gas_r,
#     32=lat_gas_r, 33=soleus_r, 37=tib_ant_r, then the mirrored left-side
#     block at the same +43 offset: 74=med_gas_l, 75=lat_gas_l, 76=soleus_l,
#     80=tib_ant_l (this session's first pass had med_gas_l off by one, at
#     75 instead of 74 -- 75 is lat_gas_l).
#   * model.states.xmin/xmax for the 'a' type are [0, 5], not PredSim's
#     default [0.05, 1] -- confirms >1 values in this block are legitimate,
#     not a sign of a wrong offset.
#   * the actual solved trajectory for med_gas_r/med_gas_l is a clean,
#     physiologically plausible push-off-timed curve, and satisfies the
#     half-gait-cycle L/R periodicity to ~1-2% (med_gas_r's last node ~=
#     med_gas_l's first node and vice versa) -- exactly what the r/l column
#     swap in `_single_muscle_activation_curve` below assumes.
MUSCLE_INDEX = {
    'med_gas': (31, 74),  # med_gas_r, med_gas_l
    'lat_gas': (32, 75),  # lat_gas_r, lat_gas_l
    'soleus': (33, 76),   # soleus_r, soleus_l
    'tib_ant': (37, 80),  # tib_ant_r, tib_ant_l
}

MUSCLE_STATE_OFFSET = 158  # gait_loading._extract_from_X's muscle_indices = range(158, 250) -- confirmed above

# Max isometric force per muscle (N), read directly off the same .osim model
# figure02.py already uses (MuscleSet.get(name).getMaxIsometricForce()),
# confirmed identical for the _r/_l pair (bilaterally symmetric model).
# Combining the triceps surae's three heads by this weight approximates each
# head's relative contribution to net plantarflexor force, since raw
# activation alone ignores that soleus (Fmax=3549 N) is >2x either
# gastrocnemius head (1558 N medial, 683 N lateral).
TRICEPS_SURAE_FMAX = {'med_gas': 1558.0, 'lat_gas': 683.0, 'soleus': 3549.0}

# The two curves this panel actually draws: 'triceps_surae' (the Fmax-weighted
# combination of the three components above) and 'tib_ant' (unweighted, a
# single muscle) -- per user request.
MUSCLE_LABELS = {'triceps_surae': 'Triceps surae', 'tib_ant': 'Tib. anterior'}
MUSCLE_LINESTYLE = {'triceps_surae': '-', 'tib_ant': '--'}


def _single_muscle_activation_curve(row, muscle_key):
    """Raw, unsmoothed full-gait-cycle activation for one muscle, reconstructed
    directly off `row['X']` -- deliberately NOT `row['activations']`.

    `gait_loading._extract_from_X` builds `row['activations']`'s second half
    as `np.concatenate([activations, activations[::-1]])`: a straight time-
    reversal of ALL 92 muscle columns at once. That's only a valid full-cycle
    reconstruction for a quantity that's genuinely symmetric in time, which a
    single muscle's own activation is not -- the correct second half (by the
    OCP's own left/right periodicity constraint) is the MIRROR muscle's first-
    half values, not this muscle's own values played backwards. This is the
    same category of bug figure02.py's RAW_COL/_extract_symmetric_angle fixes
    for the six whole-body (non left/right-paired) angles; muscles need the
    r/l column swap instead of a sign flip, exactly like `angles_unique` in
    `gait_loading._extract_from_X` already does correctly for the paired
    joint angles (hip/knee/ankle etc.) but never got applied to activations.
    Confirmed empirically against the real solved trajectory on the
    workstation this session: med_gas_r's last node and med_gas_l's first
    node (and vice versa) agree to ~1-2%, exactly the half-gait-cycle L/R
    periodicity this swap assumes.

    Uses figure02._heelstrike_shift (raw 50-node half-cycle `states`, no
    smoothing) so the result lines up sample-for-sample with row['grf']/
    row['angles']'s own heelstrike-first indexing.
    """
    idx_r, idx_l = MUSCLE_INDEX[muscle_key]
    states, _, _, heelstrike_idx = f2._heelstrike_shift(row)
    half_r = states[:, MUSCLE_STATE_OFFSET + idx_r]
    half_l = states[:, MUSCLE_STATE_OFFSET + idx_l]
    full = np.concatenate([half_r, half_l])
    return np.concatenate([full[heelstrike_idx:], full[:heelstrike_idx]])


def _triceps_surae_curve(row):
    """Fmax-weighted average of med_gas/lat_gas/soleus's own full-gait-cycle
    curves (see TRICEPS_SURAE_FMAX)."""
    total_fmax = sum(TRICEPS_SURAE_FMAX.values())
    weighted = sum(TRICEPS_SURAE_FMAX[m] * _single_muscle_activation_curve(row, m)
                   for m in TRICEPS_SURAE_FMAX)
    return weighted / total_fmax


def _muscle_activation_curve(row, panel_key):
    """Dispatch to the triceps-surae combination or a plain single muscle,
    for whichever of MUSCLE_LABELS's two entries `panel_key` is."""
    if panel_key == 'triceps_surae':
        return _triceps_surae_curve(row)
    return _single_muscle_activation_curve(row, panel_key)


# ---------------------------------------------------------------------------
# Experimental EMG reference: non-perturbed gait cycles from the Lorenz &
# van den Bogert (2024) Zenodo perturbation dataset (10.5281/zenodo.10557913,
# CC-BY 4.0, 10 subjects, level walking at 1.2 m/s nominal, discrete
# right-side belt-speed perturbations). Per user request: use only the
# non-perturbed cycles as the experimental comparator, not the earlier
# Falisse-2019 candidate (that one is itself a SIMULATED benchmark, not
# measured EMG, and was rejected as a stand-in per user instruction).
#
# Extraction (scripts/extract_zenodo_perturbation_emg.py, run this session
# against the raw ~2GB RawData.zip, not committed to this repo): real
# perturbation events are identified from RightBeltspeed itself (brief
# ~0.09s spikes to ~1.95 m/s), NOT the file's own 'Perturbationtype' column
# (a scheduled-block code spanning many seconds, not the event instant).
# Right heel-strikes come from FP2.ForY (FP1=left/FP2=right, confirmed by
# correlating each plate's loading against LHEE/RHEE marker height). Any
# gait cycle within 2s of a real perturbation event is dropped -- ~55% of
# cycles survive, 7253 clean cycles pooled across all 10 subjects. EMG is
# band-pass filtered + rectified + low-pass enveloped over each continuous
# trial (not per-cycle, to avoid edge artifacts), then each subject's own
# three muscles are normalized by that subject's own peak envelope value
# across their clean cycles (no MVC trial exists in this dataset -- this is
# the standard fallback, sometimes called "peak dynamic" normalization).
#
# Only TibialisAnterior, LateralGastrocnemius, and Soleus were recorded --
# NO medial gastrocnemius channel exists in this dataset. So the "triceps
# surae" reference curve below is a *reduced* Fmax-weighted combination of
# just lateral gastroc + soleus (re-normalized over those two components
# only, weights 683/(683+3549) and 3549/(683+3549)) -- not a like-for-like
# match to the simulated triceps_surae curve, which includes medial gastroc
# too. This is a real limitation of the available open dataset, not an
# oversight; it's the closest experimental comparator found (see this
# session's dataset survey) with all needed muscles recorded individually
# and speed matching the OCP's own 1.3 m/s walking condition.
REFERENCE_EMG_PATH = os.path.join(REPO_ROOT, 'reference_data', 'zenodo_perturbation_emg_8muscle.npz')

# Reduced two-component weights (see docstring above) -- NOT the same as
# TRICEPS_SURAE_FMAX, which includes medial gastroc for the simulated curve.
REFERENCE_TRICEPS_SURAE_FMAX = {'lat_gas': TRICEPS_SURAE_FMAX['lat_gas'], 'soleus': TRICEPS_SURAE_FMAX['soleus']}


def load_reference_emg():
    """(mean, std) per panel entry ('triceps_surae', 'tib_ant') from the
    Zenodo perturbation dataset's non-perturbed cycles, or None if the
    processed file isn't present (see REFERENCE_EMG_PATH's extraction
    script)."""
    if not os.path.exists(REFERENCE_EMG_PATH):
        print(f'[reference EMG] not found at {REFERENCE_EMG_PATH}, skipping overlay')
        return None
    data = np.load(REFERENCE_EMG_PATH)
    total_fmax = sum(REFERENCE_TRICEPS_SURAE_FMAX.values())
    weight = {'lat_gas': REFERENCE_TRICEPS_SURAE_FMAX['lat_gas'] / total_fmax,
             'soleus': REFERENCE_TRICEPS_SURAE_FMAX['soleus'] / total_fmax}
    per_subject_triceps_surae = (weight['lat_gas'] * data['per_subject_LateralGastrocnemius']
                                 + weight['soleus'] * data['per_subject_Soleus'])
    return {
        'triceps_surae': (per_subject_triceps_surae.mean(axis=0), per_subject_triceps_surae.std(axis=0)),
        'tib_ant': (data['mean_TibialisAnterior'], data['std_TibialisAnterior']),
    }


MUSCLE_PANEL_SPEED = 1.3  # m/s -- per user request, distinct from GRF_SHAPE_SPEED (1.6 m/s)


def load_muscle_activation_panel_data(speed=MUSCLE_PANEL_SPEED):
    """Per-panel-entry ('triceps_surae', 'tib_ant'), per-cost-model array of
    full-gait-cycle activation curves (one row per walking trial at `speed`)
    -- same trial selection as `load_grf_shape_panel_data` (speed-sweep,
    notebook-cleaned, walking-only) so this panel is a direct read-together
    companion of the GRF panel."""
    sweeps = {cost: _load_sweep(model_file) for cost, model_file in COST_MODEL_FILES.items()}
    curves = {muscle: {} for muscle in MUSCLE_LABELS}
    for cost, df in sweeps.items():
        sub = gm.filter_speed(df, speed, tol=f1.SPEED_TOL)
        sub = sub[sub.apply(gm.is_walking, axis=1)]
        for muscle in MUSCLE_LABELS:
            curves[muscle][cost] = np.array([_muscle_activation_curve(row, muscle)
                                             for _, row in sub.iterrows()])
    return curves


def draw_muscle_activation_panel(fig, gs_a):
    """Mean +/- SD activation over the gait cycle for the triceps surae
    (Fmax-weighted med_gas+lat_gas+soleus, solid) and tibialis anterior
    (dashed), effort vs. metabolic cost by color, plus an experimental EMG
    reference (non-perturbed cycles from the Zenodo perturbation dataset --
    see load_reference_emg). The reference is peak-normalized per subject
    (no MVC trial exists), the sim curves are raw activation [0-5 bound] --
    so this reads as a shape/timing comparison, not a calibrated-magnitude
    one, same caveat as any EMG-to-activation comparison.

    align=False (see _draw_condition_curves): unlike panel b/d's references,
    this one's 0%-event was deliberately built to already match the sim's
    own right-heel-strike convention, so the usual MAE-based realignment
    would only paper over genuine timing disagreement (checked this session
    for metabolic-cost tibialis anterior, whose near-zero curve produces an
    almost arbitrary 39%-of-cycle 'best' shift)."""
    ax = fig.add_subplot(gs_a)
    curves = load_muscle_activation_panel_data()
    reference = load_reference_emg()
    for muscle, linestyle in MUSCLE_LINESTYLE.items():
        ref = reference[muscle] if reference is not None else None
        _draw_condition_curves(ax, curves[muscle], COST_COLORS, reference=ref, linestyle=linestyle, align=False)
    ax.set_xlim(0, 100)
    ax.set_xlabel('Gait cycle [%]')
    ax.set_ylabel('Activation [-]')
    ax.text(0.98, 0.95, 'solid: triceps surae\ndashed: tib. anterior', transform=ax.transAxes,
           fontsize=BASE_FONT_SIZE * 0.8, color='0.3', va='top', ha='right')
...
# ---------------------------------------------------------------------------
# Panel a (current): cost function vs. Umberger-scored metabolic cost across
# the speed sweep. Replaces draw_muscle_activation_panel above as panel 'a'
# per user request -- that function is kept (still correct, still callable)
# in case it's wanted again, e.g. as a figure04b.py panel, but main() no
# longer wires it to the 'a' slot.
# ---------------------------------------------------------------------------

METABOLIC_COST_SWEEP_BOUNDS = (0.83, 4.0)  # m/s -- per user request

# Points-offset for each cost model's minimum-cost annotation, opposite
# corners so the two labels (whose minima sit close together in speed,
# ~1.3-1.7 m/s) don't collide -- same convention the original transition-
# speed panel (removed this session) used for its own two annotations.
# Per user request: metabolic (1.33 m/s) sits to the right and slightly
# below its dot; effort (1.63 m/s) sits above and slightly left of its dot.
_R = 75
_R_METABOLIC = 75  # both pushed further from their dots, per user request
MIN_COST_ANNOTATION_OFFSET = {
    'metabolic': (_R_METABOLIC * math.cos(math.radians(15)), -_R_METABOLIC * math.sin(math.radians(15))),
    'effort': (-_R * math.sin(math.radians(15)), _R * math.cos(math.radians(15))),
}


def load_metabolic_cost_sweep_data(bounds=METABOLIC_COST_SWEEP_BOUNDS, metric='metabolicCostUmberger'):
    """(cost -> (speeds, means, sds)) across the speed sweep (restricted to
    `bounds`), effort vs. metabolic (Bhargava) cost, scored by `metric` --
    default metabolicCostUmberger, a THIRD model neither OCP was actually
    optimized for (a cross-validation of "does optimizing metabolic cost
    directly (Bhargava) actually buy you a metabolically cheaper gait by an
    independent measure")."""
    result = {}
    for cost, model_file in COST_MODEL_FILES.items():
        df = _load_sweep(model_file)
        speeds = sorted(df['speed'].round(2).unique())
        speeds = [s for s in speeds if bounds[0] - 1e-6 <= s <= bounds[1] + 1e-6]
        means, sds = [], []
        for speed in speeds:
            vals = df.loc[np.isclose(df['speed'], speed, atol=0.005), metric].to_numpy(dtype=float)
            vals = vals[np.isfinite(vals)]
            mean, sd = f1._robust_mean_sd(vals) if len(vals) else (np.nan, np.nan)
            means.append(mean)
            sds.append(sd)
        result[cost] = (np.array(speeds), np.array(means), np.array(sds))
    return result


# ---------------------------------------------------------------------------
# Literature cost-of-transport comparators for the panel above -- all drawn
# in black (per user request), distinguished only by linestyle; no in-panel
# labels/legend entries (per user request: explained in the caption, not
# the figure). See this session's scratch comparison
# (plot/scratch/cost_sweep_with_classics.png, not part of any repo figure)
# for the full exploratory version with labels/error bars/extrapolations
# this was drawn from.
# ---------------------------------------------------------------------------

LITERATURE_COST_LINESTYLE = {
    'ralston': '-', 'abe_walk': '--', 'abe_run': '--', 'vernillo': ':',
}


def load_literature_cost_data():
    """(key -> (speeds, cot_J_kg_m)) for the four literature comparators:

    - 'ralston': Ralston (1958) walking regression Edot=29+0.0053v^2
      [cal/min/kg, v in m/min], evaluated ONLY over its own tested range
      (24.4-97.6 m/min = 0.407-1.627 m/s) -- no extrapolation here, unlike
      the scratch diagnostic's dotted extension.
    - 'abe_walk' / 'abe_run': Abe, Fukuoka & Horiuchi (2019), real
      per-subject CoT under normal gravity (reference_data/peerj-07-8290-
      s002.csv walking n=13 at 1.33-2.00 m/s, s003.csv running n=13 at
      2.00-2.44 m/s), mean across subjects at each tested speed.
    - 'vernillo': sub-elite runners' quadratic fit (PMC6225957),
      Edot(v)=0.006v^2+0.027v+0.048 [kcal/kg/min, v in m/s -- corrected
      this session from an earlier mis-extracted W/kg version], converted
      via *4184/60 to W/kg then /v to CoT, over its own tested range
      (1.78-5.14 m/s).

    Returned as (speeds, means, sds) -- sds is None for 'ralston'/'vernillo'
    (published equations, no raw per-subject data to take a spread from) and
    the real across-subject SD for 'abe_walk'/'abe_run' (n=13 each).
    All values in J/kg/m, the same convention as metabolicCostUmberger.
    """
    out = {}

    v_ralston = np.linspace(24.4, 97.6, 50)  # m/min
    cot_ralston = (29 + 0.0053 * v_ralston**2) / v_ralston * 4.184
    out['ralston'] = (v_ralston / 60, cot_ralston, None)

    for key, fname in [('abe_walk', 'peerj-07-8290-s002.csv'), ('abe_run', 'peerj-07-8290-s003.csv')]:
        abe_df = pd.read_csv(os.path.join(REPO_ROOT, 'reference_data', fname))
        speed_cols = [c for c in abe_df.columns if c != 'Participant No.']
        speeds = np.array([float(c.split()[0]) for c in speed_cols])
        means = abe_df[speed_cols].mean(axis=0).to_numpy()
        sds = abe_df[speed_cols].std(axis=0).to_numpy()
        out[key] = (speeds, means, sds)

    v_vernillo = np.linspace(1.78, 5.14, 60)
    edot_vernillo_kcal = 0.006 * v_vernillo**2 + 0.027 * v_vernillo + 0.048  # kcal/kg/min
    cot_vernillo = (edot_vernillo_kcal * 4184 / 60) / v_vernillo
    out['vernillo'] = (v_vernillo, cot_vernillo, None)

    return out


def draw_metabolic_cost_sweep_panel(fig, gs_a):
    """Umberger-scored metabolic cost across the speed sweep, effort vs.
    metabolic-cost (Bhargava) OCPs, plus four literature cost-of-transport
    comparators (all black, linestyle-only distinction, no in-panel
    labels -- see load_literature_cost_data). Minimum-cost annotations mark
    each sim curve's own cost-of-transport optimum.

    Both sim curves show the expected U-shaped minimum (~1.3-1.7 m/s), and
    metabolic-cost reads cheaper through most of the walking range -- but
    checked this session against the full sweep (not just
    METABOLIC_COST_SWEEP_BOUNDS's 4 m/s cutoff): the two curves cross around
    3 m/s, and by ~5 m/s effort-cost has become the cheaper gait, a genuine
    reversal rather than noise. That crossover sits outside this panel's own
    bounds, so it isn't visible here -- see this session's
    umberger_cost_sweep.png diagnostic (not part of this repo's figures) for
    the full 0.7-5.5 m/s picture."""
    ax = fig.add_subplot(gs_a)

    # Literature comparators drawn first / at the lowest zorder, per user
    # request -- reference data sits in the background, the sim curves (and
    # their own min-cost markers/annotations) stay visually in front.
    literature = load_literature_cost_data()
    for key, linestyle in LITERATURE_COST_LINESTYLE.items():
        speeds, cot, sds = literature[key]
        ax.plot(speeds, cot, color='k', linewidth=1.8, linestyle=linestyle, zorder=1)
        if sds is not None:
            ax.fill_between(speeds, cot - sds, cot + sds, color='k', alpha=0.12, linewidth=0, zorder=0)

    data = load_metabolic_cost_sweep_data()
    for cost, (speeds, means, sds) in data.items():
        color = COST_COLORS[cost]
        ax.plot(speeds, means, color=color, linewidth=2, label=COST_SHORT_LABEL[cost], zorder=3)
        ax.fill_between(speeds, means - sds, means + sds, color=color, alpha=0.15, linewidth=0, zorder=2)

        min_idx = int(np.nanargmin(means))
        min_speed, min_cost = speeds[min_idx], means[min_idx]
        ax.plot([min_speed], [min_cost], marker='o', color=color, markersize=6,
               markeredgecolor='k', markeredgewidth=0.8, zorder=5)
        ax.annotate(f'{min_speed:.2f} m/s', xy=(min_speed, min_cost),
                   xytext=MIN_COST_ANNOTATION_OFFSET[cost], textcoords='offset points',
                   color=color, fontsize=BASE_FONT_SIZE, ha='center',
                   arrowprops=dict(arrowstyle='-', color=color, lw=0.8))

    ax.set_xlim(*METABOLIC_COST_SWEEP_BOUNDS)
    ax.set_xlabel('Speed [m/s]')
...
def main():
    with plt.rc_context({'font.size': BASE_FONT_SIZE}):
        ...
        # a+GRF share a tighter wspace than GRF-to-b (per user request: only
        # the a/GRF gap should shrink, b should stay where it was) -- an
        # outer 1x2 [a+GRF | b] at the original wspace=0.5, with a's own
        # tighter 1x2 [a | GRF] nested inside its first column.
        ...
        ax_a = draw_metabolic_cost_sweep_panel(fig, left[0, 0])
        draw_cost_grf_panel(fig, left[0, 1])
        ...
        c_col = GridSpecFromSubplotSpec(3, 1, subplot_spec=top[0, 1], height_ratios=[1, 3, 1])
        ...
        fig.savefig(os.path.join(out_dir, 'figure04a.png'), dpi=500, bbox_inches='tight')
        fig.savefig(os.path.join(out_dir, 'figure04a.pdf'), bbox_inches='tight', dpi=500)
    print('Wrote plot/figure04a.png and plot/figure04a.pdf')
    if _OUTLIER_LOG:
    ...
    shutil.copyfile(os.path.join(out_dir, 'figure04a.png'), os.path.join(paper_repo_root, 'figure04a.png'))
    f1._shrink_pdf_with_ghostscript(
        os.path.join(out_dir, 'figure04a.pdf'),
        os.path.join(paper_repo_root, 'figure04a.pdf'),
        f1.PRINTED_WIDTH_IN,
    )
